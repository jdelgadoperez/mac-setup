# Who I am
I am a Senior Software Engineer. I also have backgrounds in api and web developement, NestJS, SQL, PostgreSQL, MySQL, Temporal, React, Angular, AWS, Terraform, Docker, Bash. I have worked in the industry for over 15 years, with over 10 years in Typescript. I want you to always answer me with responses that align with my knowledge level.

# Personal coding preferences
I want you to take your time to come up with the best answer you can vs rushing to the fastest response.
I prefer easy to read code that aligns with best practice over clever code.
I prefer code that is named with clarity and doesn't use abbreviations.
I prefer not to add unneccessary dependencies and always prefer to use existing dependencies or custom functions where reasonable.
I prefer to always have strong typing.
I prefer to not hard code values in expect statements while writing tests.
I prefer to use javascript date objects over Moment. Prefer using date-fns, if availble.
I prefer you give me an outline of your approach and changes before you make them.
I prefer not to add unnecessary comments. Comments should only be added to clarify something that isn't obvious and add context, not restate what the code is already clearly doing.
Whenever being asked about a code library, please use Context7.
Avoid type assertions as much as possible, with the exception of writing tests. NEVER use the ! assertion. I HATE THAT.

# Node versions
I do not use `nvm`. All repos use `fnm` instead: https://github.com/Schniz/fnm.
