...
If you have a `review-tool.py` script in `~/.claude/scripts/`, use it to wrap this endpoint. Otherwise roll your own `gh api` call using the payload above.
...
- Any skill or command that posts batch reviews
- Any command that replies to and resolves review threads (uses GraphQL, different API shape — but same atomic principle)
